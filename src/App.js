...
class App extends Component {
  ...
  getRandomPhrase(){
    ...
    var random = Math.floor(Math.random() * Math.floor(835));
    return DICT[random]
  }
  ...
  checkFinish(phrase, usedLetters)
  {
      var hiddenPhrase = this.computeDisplay(phrase, usedLetters) 
      return !hiddenPhrase.includes("_ ")
  }
  ...
  handleKeyClick = (value) => {
    ...
    if(!this.state.userLetters.includes(value)){

      const newUserLetters= this.state.userLetters;
      newUserLetters.push(value)
      this.setState({ userLetters: newUserLetters })
    }

    var newPoints = this.state.points
    if(this.state.phrase.includes(value))
    {
  ...
  render() {
    return (
      <div>
        Guesses: {this.state.guesses}<br/>
        Points: {this.state.points}
        <HiddenPhrase hiddenPhrase={this.computeDisplay(this.state.phrase, this.state.userLetters)}/>
        <br/>
        <Keyboard checkClickEvent={this.handleKeyClick} selected={this.state.userLetters} finish={this.checkFinish(this.state.phrase, this.state.userLetters)}/>
      </div>
    );
  }
}
